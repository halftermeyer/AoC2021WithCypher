...
// CREATE CHAINED LIST OF MEASUREMENT AS A GRAPH
LOAD CSV FROM "file:///input_1.txt" AS val
...
// COUNT MATCHING SLIDING WINDOWS PAIRS
MATCH (first)-[:NEXT]->(second)-[:NEXT]->(third)-[:NEXT]->(fourth)
WHERE first.depth < fourth.depth
RETURN count(*);
