...
LOAD CSV FROM 'file:///input_12.txt' AS line
WITH split(line[0], '-') AS vertices
...
CALL apoc.periodic.commit(
    'MATCH (:Small)--(small_small_leaf:Small)
    WHERE apoc.node.degree(small_small_leaf, "CONNECTED_TO") = 1
    AND NOT small_small_leaf.name IN ["start", "end"]
    DETACH DELETE small_small_leaf
    WITH count(small_small_leaf) AS limit
    RETURN limit',
...
//compute part 1
MATCH p = (s:Cave {name:"start"})-[:CONNECTED_TO*0..6]-(e:Cave {name:"end"})
WHERE all(c IN nodes(p) WHERE apoc.coll.occurrences(nodes(p), c) = 1)
RETURN count(p);
